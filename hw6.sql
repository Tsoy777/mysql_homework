...
select
		case
			when to_user_id = 1 and from_user_id in (select target_user_id
			...
			then from_user_id
			when from_user_id = 1 and to_user_id in (select target_user_id
													from friend_requests
													where initiator_user_id = 1 and status = "approved"
													union
													select initiator_user_id 
...
-- я не смогла вставить запрос выше в запрос ниже, потому что функция limit в in() не поддерживается в моей версии mysql, 
-- поэтому я переписала результаты запроса
select 
	sum((select 
		count(*)
	from likes
	where media_id in (select id from media where user_id in (84,78,45,94,57,93,17,7,85,63)))) as like_count
from profiles
where user_id = 1;
...
select
	...
	(select count(*)
	from friend_requests
	where (initiator_user_id = cur_id or target_user_id = cur_id) and status = "approved") as friend_count,
	(select count(*)
	from messages
	where (from_user_id = cur_id or to_user_id = cur_id)) as messages_count,
	(select count(*)
	from likes
	where user_id = cur_id) as likes_count,
	(select count(*)
	from media 
	where user_id = cur_id) as media_count,
	(select count(*)
	from users_communities
	where user_id = cur_id) as communities_count
from profiles
order by friend_count, messages_count, likes_count, media_count, communities_count limit 10;
